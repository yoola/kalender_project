...
def every_change_24_to_00_end(test_day, test_day_start, test_day_end):
    ...
    splitted_day = split_interval_days(test_day)
...
def is_same_day_intersection(test_day_start, test_day_end, day_start, day_end):
    if day_end >= test_day_end >= day_start >= test_day_start:
        return True
    if test_day_end >= day_end >= test_day_start >= day_start:
        return True
...
def join_intervall_days(test_day, test_day_start, test_day_end, list_days, day_start, day_end):
    if len(list_days) == 2:
        ...
        if join_days(test_day, test_day_start, test_day_end, list_days[1], day_start,
                     day_end) == "join_with_previous_day":
...
def check_intersect_with_every_list(test_day, test_day_start, test_day_end, List_days):
    ...
    if not List_days:
        ...
        for i in range(0, len(List_days)):
            ...
            list_days = split_interval_days(List_days[i][0])

            print("Splitted days: ", list_days)
            if len(list_days) == 2:

                if not (join_days(test_day, test_day_start, test_day_end, list_days[0], List_days[i][1],
                                  List_days[i][2]) == "dont_join"
                        and join_days(test_day, test_day_start, test_day_end, list_days[1], List_days[i][1],
                                      List_days[i][2]) == "dont_join"):
                    ...
                    if not_set:
                        print("First intersection.\n")
                        List_days[i] = join_intervall_days(test_day, test_day_start, test_day_end, list_days,
                                                           List_days[i][1], List_days[i][2])
                    ...
                    # print("First intersect test_range: ", test_range)
                        ...
                        print("More intersections.\n")
                        List_days[index_set] = join_intervall_days(test_day, test_day_start, test_day_end, list_days,
                                                                   List_days[i][1], List_days[i][2])
